...
/** A Shopify colour not in the local design map still needs a swatch and a name. */
function getColorInfo(colorway: string) {
  const color = COLORWAYS[colorway as keyof typeof COLORWAYS];
  return color ? { name: color.name, hex: color.hex } : { name: colorway, hex: "#3A3A3A" };
}
...
export function ProductCard({ product, index = 0, feature = false }: ProductCardProps) {
  ...
  // A Shopify product can arrive between fetch and re-render with no variant
  // for the index a colour swap left behind — fall back before indexing sizes.
  const variant = product.variants[variantIndex] ?? product.variants[0];
  const saved = wishlist.includes(product.slug);
  const stock = stockLevel(product);
  const secondaryMedia = product.shopifyImages?.[1]?.url;
    ...
    >
      ...
      <div className="relative overflow-hidden bg-graphite">
        ...
        <div className="pointer-events-none absolute left-3 top-3 flex flex-col items-start gap-1.5">
          ...
          {product.compareAt && <span className="badge badge-purple">Save {formatPrice(product.compareAt - product.price, currency)}</span>}
        </div>
        ...
        <button
          ...
          className={[
            ...
            saved
              ? "text-purple-bright opacity-100"
        ...
        >
          {added ? (
            <p className="flex items-center justify-center gap-2 py-4 font-mono text-micro uppercase tracking-[0.16em] text-purple-bright">
            ...
            <button
              ...
              className="w-full py-4 font-mono text-micro uppercase tracking-[0.18em] text-bone transition-colors duration-300 hover:text-purple-bright"
            >
              Quick add
            </button>
          )}
        ...
        {product.variants.length > 1 && (
          <fieldset className="mt-3.5">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="ml-1 text-micro text-ash">{getColorInfo(variant.colorway).name}</span>
            </div>
          </fieldset>
        )}

        {stock === "out" && (
          <p className="mt-2 font-mono text-micro uppercase tracking-[0.14em] text-ash">Sold out</p>
        )}
      </div>
    </article>
  );
