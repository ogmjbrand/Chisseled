        ...
        <div className="grid gap-3 lg:grid-cols-3">
          ...
          >
            ...
            <div className="relative z-[3] flex h-full flex-col justify-end p-7">
              <p className="eyebrow mb-3 text-emerald-bright">{ATHLETES[3].discipline}</p>
          ...
          {quotes.map((q, i) => (
            ...
            >
              <div>
                <div className="mb-4 flex items-center gap-0.5" aria-label={`${q.rating} out of 5`}>
                  {Array.from({ length: 5 }).map((_, s) => (
                    <StarMark
                      key={s}
                      className={s < q.rating ? "size-3.5 text-emerald-bright" : "size-3.5 text-iron"}
                      filled={s < q.rating}
                    />
                  ))}
                </div>
              ...
              <figcaption className="mt-6 flex items-baseline justify-between gap-3 border-t border-bone/10 pt-4">
                ...
                <Link
                  href={`/product/${q.slug}`}
                  className="truncate font-mono text-micro uppercase tracking-[0.12em] text-ash transition-colors hover:text-emerald-bright"
                >
                  {q.product}
                </Link>
              </figcaption>
